...
mkFiles make name cf =
    do -- Create the package directories if necessary.
       ...
       writeFileRep (dirBase ++ name ++ ".lex") lex
...
makefile name dirBase dirAbsyn absynFileNames =
    ...
    [
     ...
     "",
     "%.class: " ++ "%.java",
     "\t${JAVAC} ${JAVA_CFLAGS} $*.java",
     "",
     dirBase ++ "Visitable.class: " ++ dirBase ++ "Visitable.java",
     "\t${JAVAC} ${JAVA_CFLAGS} " ++ dirBase ++ "Visitable.java",
     "",
     dirBase ++ "Visitor.class: " ++ dirBase ++ "Visitor.java",
     "\t${JAVAC} ${JAVA_CFLAGS} " ++ dirBase ++ "Visitor.java",
     "",
     dirBase ++ name ++ ".lex.java: " ++ dirBase ++ name ++ ".lex",
     "\t${JAVA} ${JLEX} " ++ dirBase ++ name ++ ".lex",
     "",
     -- FIXME
     dirBase ++ "sym.java" ++ " " ++ dirBase ++ "parser.java: " ++ dirBase ++ name ++ ".cup",
     "\t${JAVA} ${CUP} ${CUPFLAGS} " ++ dirBase ++ name ++ ".cup ; mv sym.java parser.java " ++ dirBase,
     "",
     dirBase ++ "Yylex.class: " ++ dirBase ++ name ++ ".lex.java "
       ++ dirBase ++ "sym.java",
     "\t${JAVAC} ${JAVA_CFLAGS} " ++ dirBase ++ name ++ ".lex.java",
     "",
     dirBase ++ "Test.class: "
       ++ unwords (map (dirBase ++) ["Test.java",
				     ...
				     "parser.class",
				     "sym.class"]),
     "\t${JAVAC} ${JAVA_CFLAGS} " ++ dirBase ++ "Test.java",
     "",
     "" ++ name ++ ".dvi: " ++ name ++ ".tex",
     "\t${LATEX} " ++ name ++ ".tex",
     "",
...
-- FIXME
     ...
     "\t rm -f " ++ unwords (map (dirBase ++) [name ++ ".lex",
					       name ++ ".cup",
					       name ++ ".lex.java",
					       "sym.java", 
					       "parser.java",
