...
rules cf = oneRule $ concat [
        ...
        if null (symbols cf) then [] 
            else if allSymbolsOfLengthOne cf -- matching var will have type char, not string...
                    then ["rsyms {let id = lexeme lexbuf in try Hashtbl.find symbol_table (String.make 1 id) with Not_found -> failwith (\"internal lexer error: reserved symbol \" ^ (String.make 1 id) ^ \" not found in hashtable\")}"]
    ...
    where
        ...
        allSymbolsOfLengthOne cf = and $ map (\x -> length x == 1) (symbols cf)
