...
makeAllGADT :: Bool -> Bool -> Bool -> Bool -> Bool -> Int 
	   -> Maybe String -- ^ The hierarchical package to put the modules
	                   --   in, or Nothing.
	   -> String -> FilePath -> IO ()
makeAllGADT m a1 d ss g x p n file = do
  ...
  if isOK then do
    ...
    when (not (null dir)) $ do
			    ...
			    prepareDir dir
    writeFileRep (absFile opts) $ cf2Abstract absMod cf composOpMod
    ...
    if alex1 opts then do
		    ...
		    writeFileRep (alexFile opts) $ cf2alex2 lexMod errMod shareMod (shareStrings opts) cf
                    putStrLn "   (Use Alex 2.0 to compile.)"
    writeFileRep (happyFile opts) $ 
		 cf2HappyS parMod absMod lexMod errMod (glr opts) cf
    ...
    when (shareStrings opts) $ writeFileRep (shareFile opts)    $ sharedString shareMod cf
