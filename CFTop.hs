{-
    ...
    Patrik Jansson, Antti-Juhani Kaijanaho, Michael Pellauer, Aarne Ranta
...
mkOne xx = do
      ...
      if checkUsage False [c, cpp, cpp_stl, csharp, java, haskell, profile] then
       ...
       if (isCF (reverse file)) then 
        do 
         ...
         case () of
           ...
           _ | cpp_stl-> makeSTL make linenumbers name file
	   ...
	   _ | haskellGADT -> makeAllGADT make alex1or2 inDir alex2StringSharing glr xml inPackage name file
           _          -> makeAll make alex1or2 inDir alex2StringSharing glr xml inPackage name multi file
         if (make && multi) 
